...
sql_to_s3_order_items = SqlToS3Operator(
    task_id="sql_to_s3_order_items4",
    query="SELECT * FROM order_items;",
    s3_bucket=bucket_name,
    s3_key=bucket_subfolder + "order-items4-" + str(current_date) + ".csv",
    replace=True,
...
sql_to_s3_order_payments = SqlToS3Operator(
    task_id="sql_to_s3_order_payments4",
    query="SELECT * FROM order_payments;",
    s3_bucket=bucket_name,
    s3_key=bucket_subfolder + "order-payments4-" + str(current_date) + ".csv",
    replace=True,
    sql_conn_id=rdsmysql_conn,
    aws_conn_id=aws_conn,
    file_format='csv',
...
sql_to_s3_orders = SqlToS3Operator(
    task_id="sql_to_s3_orders4",
    query="SELECT * FROM orders;",
    s3_bucket=bucket_name,
    s3_key=bucket_subfolder + "orders4-" + str(current_date) + ".csv",
    replace=True,
    sql_conn_id=rdsmysql_conn,
    aws_conn_id=aws_conn,
    file_format='csv',
